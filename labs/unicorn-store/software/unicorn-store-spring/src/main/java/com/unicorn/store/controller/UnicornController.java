    ...
    public ResponseEntity<Unicorn> createUnicorn(@Valid @RequestBody Unicorn unicorn) {
        try {
            return ResponseEntity
                ...
                .body(unicornService.createUnicorn(unicorn));
        } catch (IllegalArgumentException e) {
            logger.warn("Invalid unicorn data: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage(), e);
        } catch (Exception e) {
            logger.error("Error creating unicorn: {}", e.getMessage(), e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Error creating unicorn", e);
        }
    }
